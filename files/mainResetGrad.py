...
def getOptimizer(name, net, learningRate, mom, wd, useNesterov):
    if name == 'sgd':
        optimizer = optim.SGD(net.parameters(), lr=learningRate, momentum=mom, weight_decay=wd, nesterov=useNesterov)
    elif name == 'adagrad':
        optimizer = optim.Adagrad(net.parameters(), lr=learningRate)
    elif name == 'adadelta':
        optimizer = optim.Adadelta(net.parameters(), lr=learningRate)
    elif name == 'adam':
...
if __name__ == '__main__':
   ...
   # Data loading code
    ...
    # Optimizer
    optimizer = getOptimizer(args.optimizer, net, args.lr, args.momentum, args.weight_decay, False)
    print(optimizer)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        if not args.steady and int(epoch) in [60, 120, 160]:
           # LRD schedule
            if args.LRD:
                lr = lr / args.factor
                optimizer = getOptimizer(args.optimizer, net, lr, args.momentum, args.weight_decay, False)
